...
**Ritix** is a game development project currently being produced in Godot 4.7.2, with the intention of becoming the base systems for two seperate games in the future. While extremely bare right now, the intention is to build the shared systems between the two potential games, mostly focusing on the player systems and combat systems. Ritix is a third person 3D combat sandbox in it's current state.

So far, there are a few major differences between **Ritix** and other projects such as [Spellbound 2026](../spellbound-2026). Spellbound used Configs and RefCounted objects to create and write it's save data. While it worked for it's intended purpose, it was clunky and over-engineered. Ritix uses a a custom resource to hold save data which is a lot simpler, and frankly what Godot recomends.

Another HUGE difference with Ritix is the player controller scripts. [Spellbound 2024](../spellbound-2024) had a monolith script as it's player controller. Multiple `if else` chains that were 8+ statements long. [Spellbound 2026](../spellbound-2026) improved upon this with classes like the `PlayerCameraManager` and `PlayerInputManager`, and dedicated `Stat` and `StatModifier` objects to decouple operations from eachother. There was, however, still huge room for improvement.
